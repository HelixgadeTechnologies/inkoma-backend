      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {choices.map((choice, idx) => (
          <button
            ...
            className="group relative flex flex-col justify-between rounded-2xl border border-stone-200 bg-white hover:bg-[#D4AF37]/5 p-5 text-left transition-all duration-300 hover:border-[#D4AF37] hover:shadow-md hover:-translate-y-0.5 active:scale-[0.99]"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <ArrowRight className="h-4 w-4 text-stone-400 group-hover:text-[#D4AF37] group-hover:translate-x-1 transition-all" />
            </div>

            <p className="mt-3 text-base font-serif font-bold text-stone-900 group-hover:text-[#D4AF37] transition-colors leading-snug">
              {choice.label}
            </p>

            {choice.consequenceHint && (
              <div className="mt-3 flex items-center gap-1.5 text-xs text-stone-600">
                <Compass className="h-3.5 w-3.5 text-[#D4AF37] shrink-0" />
