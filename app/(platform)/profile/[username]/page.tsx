...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-4xl mx-auto">
      ...
      <div className="relative rounded-3xl border border-folklore-amber/30 bg-gradient-to-r from-folklore-obsidian via-folklore-indigo to-folklore-night p-6 sm:p-8 overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 relative z-10">
          <Avatar
            fallback="KA"
            className="h-20 w-20 text-xl border-2 border-folklore-amber shadow-xl"
          />
          ...
          <div className="flex-1 space-y-2">
            ...
            <p className="text-xs text-folklore-gold font-mono">@{author.username}</p>
            ...
            <div className="flex items-center gap-4 text-xs text-neutral-400 pt-1">
              <span><strong className="text-white">{author.followersCount}</strong> Disciples</span>
              <span><strong className="text-white">{author.totalReadsCount.toLocaleString()}</strong> Listeners</span>
              <span className="flex items-center gap-1">
                <Calendar className="h-3 w-3" /> Member since Oct 2025
              </span>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-serif font-bold text-white flex items-center gap-2">
          <Award className="h-4 w-4 text-folklore-gold" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {author.badges.map((badge) => (
            <div
              key={badge.id}
              className="flex items-start gap-3 rounded-2xl border border-folklore-amber/20 bg-folklore-obsidian/70 p-4 transition-all hover:border-folklore-amber/40"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-folklore-gold/15 text-folklore-gold">
                <Sparkles className="h-5 w-5" />
              </div>
              <div className="space-y-0.5">
                <h4 className="font-serif text-sm font-bold text-white">{badge.name}</h4>
                <p className="text-[11px] text-neutral-400 leading-relaxed">{badge.description}</p>
                <span className="text-[10px] text-folklore-amber font-semibold block pt-1">
      ...
      <div className="space-y-4">
        ...
        <Card className="border-folklore-amber/20 bg-folklore-obsidian/80">
          <CardHeader>
            <div className="flex items-center justify-between">
              <Badge variant="gold">Ashanti/Akan</Badge>
              <span className="text-xs text-neutral-400">14 Choice Paths</span>
            </div>
            <CardTitle className="text-lg mt-2 text-white">Anansi and the Pot of Wisdom</CardTitle>
          </CardHeader>
          <CardContent className="flex items-center justify-between pt-0">
            <div className="flex items-center gap-4 text-xs text-neutral-400">
              <span className="flex items-center gap-1">
                <Eye className="h-3.5 w-3.5" /> 38.4K reads
              </span>
              <span className="flex items-center gap-1">
